...
def download_file(decoded_msg, nsocket):
	...
	if socket.gethostname() == 'datanode1':
		client_datanode_addr = ("10.0.0.12", 20002)

	if socket.gethostname() == 'datanode2':
		client_datanode_addr = ("10.0.0.13", 20002)

	client_datanode_socket.bind(client_datanode_addr)
	client_datanode_socket.listen(1000)

	print('[!] Waiting for the client connection')

	conn, addr = client_datanode_socket.accept()

	print('Start downloading file {}'.format(file_path))

	file = open(file_path[1:], "wb")
	...
	# info_msg = str(client_datanode_socket.recv(1024), 'utf8')
